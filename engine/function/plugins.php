...
function plugin_repository_cached(Cache $cache, bool $refresh): ?array {
 ...
 * and {"plugins": [...]}. Returns null (with $error set) on any failure.
 */
function plugin_repository_fetch_raw(string $indexUrl, bool $refresh, ?string &$error): ?array {
	if ($refresh) {
...
/** Raw catalogue entries, validated and reshaped into the plugin-list format the rest of the admin panel expects. */
function plugin_repository_normalize_entries(array $data): array {
	$plugins = array();
...
function plugin_repository_list(bool $refresh = false): array {
	...
	$cached = plugin_repository_cached($cache, $refresh);
	if ($cached !== null) {
		return array('plugins' => $cached, 'error' => '', 'cached' => true);
	}

	$error = null;
	$data  = plugin_repository_fetch_raw($cfg['index'], $refresh, $error);
	if ($data === null) {
		return array('plugins' => array(), 'error' => (string)$error);
	}

	$plugins = plugin_repository_normalize_entries($data);
