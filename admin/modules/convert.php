...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'upload_download') {
		if (!in_array($source, ['auto', 'myaac', 'gesior'], true)) {
			acp_flash_error('Choose Auto, MyAAC or Gesior2012 for the uploaded SQL.');
			acp_redirect('convert');
		}

		if (!isset($_FILES['sql_file']) || !is_uploaded_file($_FILES['sql_file']['tmp_name'])) {
			acp_flash_error('Upload a MyAAC or Gesior SQL dump first.');
			acp_redirect('convert');
		}

		$size = (int)($_FILES['sql_file']['size'] ?? 0);
		if ($size <= 0 || $size > 256 * 1024 * 1024) {
			acp_flash_error('The SQL dump must be between 1 byte and 256 MB.');
			acp_redirect('convert');
		}
	...
	if ($action === 'import_converted') {
		if (!isset($_FILES['converted_sql_file']) || !is_uploaded_file($_FILES['converted_sql_file']['tmp_name'])) {
			acp_flash_error('Upload a converted ZnoteX SQL file first.');
			acp_redirect('convert');
		}

		$size = (int)($_FILES['converted_sql_file']['size'] ?? 0);
		if ($size <= 0 || $size > 256 * 1024 * 1024) {
			acp_flash_error('The converted SQL file must be between 1 byte and 256 MB.');
			acp_redirect('convert');
		}
		...
		if (strpos($sql, '-- ZnoteX uploaded SQL conversion') === false || strpos($sql, 'znote_legacy_tables') === false) {
			acp_flash_error('This importer only accepts SQL files generated by this converter.');
			acp_redirect('convert');
		}
		...
		if ($report['ok']) {
			...
			acp_flash_success('Converted SQL imported. Statements executed: ' . (int)$report['executed'] . '.');
		} else {
			$error = $report['errors'][0] ?? ['statement' => 0, 'message' => 'Unknown SQL error.'];
			acp_flash_error('Import stopped at statement #' . (int)$error['statement'] . ': ' . h((string)$error['message']));
		}
		acp_redirect('convert');
	}

	acp_flash_error('Unknown converter action.');
	acp_redirect('convert');
}
...
<div class="acp-flash acp-flash--info">
	...
	<span>
		Upload a MyAAC or Gesior2012 SQL dump here. ZnoteX will generate a converted SQL file
		to download. After checking it, use the import section below to apply that generated SQL.
	</span>
</div>

<section class="acp-card">
	<header class="acp-card-head">
		<h2>Convert Uploaded SQL</h2>
		<p>MyAAC or Gesior2012 dump in, ZnoteX SQL out</p>
	</header>
	<div class="acp-card-body">
		<form method="post" enctype="multipart/form-data" data-convert-form>
			...
			<div class="acp-field">
				<label class="acp-label" for="source">Source AAC</label>
				<select class="acp-select" id="source" name="source">
					<option value="auto">Auto detect</option>
					<option value="myaac">MyAAC</option>
					<option value="gesior">Gesior2012</option>
				</select>
				<p class="acp-hint">Auto detect uses table names like <code>myaac_news</code> or <code>z_news_big</code>.</p>
			</div>

			<div class="acp-field">
				<label class="acp-label" for="sql_file">SQL dump</label>
				<input class="acp-input" id="sql_file" name="sql_file" type="file" accept=".sql,text/sql,text/plain" required>
				<p class="acp-hint">Maximum upload handled by this tool: 256 MB, also limited by PHP upload settings.</p>
			</div>

			<div class="acp-actions">
				<button class="acp-btn acp-btn--blue" type="submit">
					<i class="fa fa-download"></i> Convert and download SQL
				</button>
			</div>
		</form>
	</div>
...
<section class="acp-card">
	<header class="acp-card-head">
		<h2>Import Converted SQL</h2>
		<p>Apply a converted ZnoteX SQL file to this database</p>
	</header>
	<div class="acp-card-body">
		<form method="post" enctype="multipart/form-data" data-convert-form>
			<?= acp_csrf_field() ?>
			...
			<div class="acp-field">
				<label class="acp-label" for="converted_sql_file">Converted SQL</label>
				<input class="acp-input" id="converted_sql_file" name="converted_sql_file" type="file" accept=".sql,text/sql,text/plain" required>
				<p class="acp-hint">Only SQL generated by this converter is accepted. Back up the database before importing.</p>
			</div>

			<div class="acp-actions">
				<button class="acp-btn acp-btn--green" type="submit">
					<i class="fa fa-upload"></i> Import converted SQL
				</button>
			</div>
		</form>
		...
		<?php if (is_array($importReport)): ?>
			<div class="acp-convert-terminal<?= empty($importReport['errors']) ? '' : ' acp-convert-terminal--error' ?>">
				...
				<div>$ file: <?= h((string)($importReport['file'] ?? 'converted.sql')) ?> (<?= number_format((int)($importReport['size'] ?? 0)) ?> bytes)</div>
				<div>$ started: <?= h((string)($importReport['started'] ?? '')) ?></div>
				<div>$ finished: <?= h((string)($importReport['finished'] ?? '')) ?></div>
				<div>$ statements: <?= (int)($importReport['executed'] ?? 0) ?> / <?= (int)($importReport['total'] ?? 0) ?></div>
				<div>$ status: <?= !empty($importReport['ok']) ? 'ok' : 'error' ?></div>
				<div>$ current admin account protected: <?= !empty($importReport['protected_account']) ? 'yes' : 'no' ?></div>
				<div>$ remapped account IDs: <?= (int)($importReport['remapped']['accounts'] ?? 0) ?></div>
				<div>$ remapped player IDs: <?= (int)($importReport['remapped']['players'] ?? 0) ?></div>
				<?php if (!empty($importReport['rolled_back'])): ?>
					<div>$ rollback: attempted</div>
				<?php endif; ?>
				<?php foreach (($importReport['errors'] ?? []) as $error): ?>
					<div>$ error statement #<?= (int)($error['statement'] ?? 0) ?>: <?= h((string)($error['message'] ?? 'Unknown SQL error.')) ?></div>
					<?php if (!empty($error['sql'])): ?>
						<div>$ sql: <?= h((string)$error['sql']) ?></div>
					<?php endif; ?>
				<?php endforeach; ?>
			</div>
		<?php endif; ?>
...
<section class="acp-card">
	...
	<div class="acp-card-body">
		<p>
			The generated file creates the extra ZnoteX conversion tables, restores the original
			source tables with their rows and custom columns, inserts Znote-compatible rows for
			known content, and archives the full original dump data.
		</p>
		<p>
			MyAAC imports <code>myaac_news</code>, <code>myaac_changelog</code>,
			<code>myaac_forum_boards</code>, <code>myaac_forum</code>, <code>myaac_pages</code>,
			<code>myaac_gallery</code>, <code>myaac_menu</code> and legacy config tables.
			Gesior2012 imports <code>z_news_big</code>, <code>z_news_tickers</code> and
			<code>z_forum</code> when they exist. Config without a direct ZnoteX equivalent is
			preserved under <code>legacy:*</code> keys in <code>znote_config</code>.
			All tables from the uploaded dump are also archived row-by-row into
			<code>znote_legacy_tables</code> and <code>znote_legacy_rows</code>, so custom columns
			and custom tables are not lost even when ZnoteX does not have a direct feature for them.
		</p>
	</div>
</section>
